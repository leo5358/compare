...
void setup() {
  ...
  divisibleGame = new DivisibleGame();
}
...
void mousePressed() {
  ...
  // 判斷是否點擊關閉按鈕
  if (showPopup &&
    mouseX > width / 2 + 290 && mouseX < width / 2 + 350 &&
    mouseY > height / 2 - 200 && mouseY < height / 2 - 140) {
    showPopup = false; // 關閉彈出視窗
    pop = false;
  }
}
